...
const SearchModalSalesReferenceNo = ({
  setData,
  props,
  label,
  name,
  ...
  const handleClick = (item) => {
    props.searchProduct = `${item.sales_reference_no}`;
    setOnFocus(false);
    setData(item);
          ...
          >
            {isLoading || isFetching ? (
              ...
              result?.data.map((item, key) => (
                ...
                >
                  {item.sales_reference_no} ({item.customer_name})
                </button>
              ))
            ) : (
...
export default SearchModalSalesReferenceNo;
